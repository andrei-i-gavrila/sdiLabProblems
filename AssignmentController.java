...
public class AssignmentController {
    ...
    public String bestStudent() {
        ...
        Map<String, Integer> freqs = new HashMap<>();
        for (Assignment a : assignments) {
            freqs.merge(a.getStudentRegistrationNumber(),
                    1,
                    Integer::sum);
        }

        Map<String, Double> grades = new HashMap<>();
        for (Assignment a : assignments) {grades
                .merge(a.getStudentRegistrationNumber()
                    ,a.getGrade(),
                    Double::sum);
        }

        grades.forEach((k,v) -> v=v/(freqs.get(k)));
        Double maxvalue = grades.values().stream().max(Comparator.comparingDouble(x-> (Double) x)).get();
        return grades.entrySet().stream().filter(x -> x.getValue().equals(maxvalue)).findFirst().get().getKey();
